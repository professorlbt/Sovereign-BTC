...
const Auth = ({ onLogin }: { onLogin: (user: any) => void }) => {
  ...
  const handleAuth = (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      if (email === 'admin@sovereign.btc' && password === 'admin') {
        onLogin({ email, id: 'admin-root', isAdmin: true, status: 'Root Authority' });
        navigate('/admin');
        return;
      }

      const student = regs.find((r: any) => 
        (r.email === email || r.platformEmail === email) && r.password === password
      );

      if (student) {
        onLogin({ 
          email: student.platformEmail || student.email, 
          id: student.id, 
          name: student.name, 
        ...
        setError("Identity not found or Security Key incorrect. Verify your credentials.");
      }
    } else {
      ...
      const newEntry = {
        ...
        email,
        password,
        name: mode === 'register-premium' ? `${firstName} ${lastName}` : 'Simple Observer',
        status: mode === 'register-premium' ? 'Pending' : 'Simple',
        platformEmail: '', 
        type: mode === 'register-premium' ? 'Premium' : 'Simple',
        timestamp: new Date().toISOString()
      };

      if (regs.some((r: any) => r.email === email)) {
        setError("This identity already exists in our ledger.");
        return;
      }
      ...
      onLogin({ email: newEntry.email, id: newId, name: newEntry.name, status: newEntry.status, type: newEntry.type });
        ...
        <div className="p-8">
          {mode === 'login' && (
            <form onSubmit={handleAuth} className="space-y-6">
              <InputField label="Terminal Handle (Email)" type="email" value={email} onChange={setEmail} icon={Mail} placeholder="id@sovereign.btc" />
              <InputField label="Security Key (Password)" type="password" value={password} onChange={setPassword} icon={Lock} placeholder="••••••••" />
              <SubmitButton label="Initialize Access" icon={LogIn} />
            </form>
          )}
          ...
          {mode === 'register-simple' && (
            <form onSubmit={handleAuth} className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              <div className="flex items-center gap-2 text-zinc-500 mb-6">
                <button type="button" onClick={() => setMode('register-selection')} className="hover:text-white"><ArrowRight size={16} className="rotate-180" /></button>
                <span className="text-[10px] font-black uppercase tracking-widest">Observer Registration</span>
              </div>
              <InputField label="Professional Email" type="email" value={email} onChange={setEmail} icon={Mail} placeholder="name@email.com" />
              <InputField label="Create Security Key" type="password" value={password} onChange={setPassword} icon={Lock} placeholder="••••••••" />
              <SubmitButton label="Establish Connection" icon={UserPlus} />
            </form>
          )}
          ...
          {mode === 'register-premium' && (
            <form onSubmit={handleAuth} className="space-y-8 animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="grid grid-cols-2 gap-4">
                <InputField label="First Name" type="text" value={firstName} onChange={setFirstName} icon={User} placeholder="Jane" />
                <InputField label="Last Name" type="text" value={lastName} onChange={setLastName} icon={User} placeholder="Doe" />
              </div>
              <InputField label="Contact Email" type="email" value={email} onChange={setEmail} icon={Mail} placeholder="jane.doe@pro.com" />
              <InputField label="Security Key" type="password" value={password} onChange={setPassword} icon={Lock} placeholder="••••••••" />
              ...
              <div className="space-y-4 pt-4 border-t border-zinc-800">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <RuleCheck label="BTCUSDT Perpetual only execution" active={checks.btcOnly} onToggle={() => toggleCheck('btcOnly')} />
                  <RuleCheck label="London → New York session only (5-9 PM PKT)" active={checks.sessionOnly} onToggle={() => toggleCheck('sessionOnly')} />
                  <RuleCheck label="Risk defined exclusively in R-multiples" active={checks.rMultiples} onToggle={() => toggleCheck('rMultiples')} />
                  <RuleCheck label="Zero expectation of signals or guaranteed profit" active={checks.noSignals} onToggle={() => toggleCheck('noSignals')} />
                  <RuleCheck label="Discipline and survival over net-profit focus" active={checks.survivalFirst} onToggle={() => toggleCheck('survivalFirst')} />
                  <RuleCheck label="Acceptance of full personal risk responsibility" active={checks.responsibility} onToggle={() => toggleCheck('responsibility')} />
                </div>
              </div>

              <div className="space-y-4">
                <SubmitButton 
                  label="Initialize Enrollment" 
                  icon={ShieldCheck} 
                  disabled={!allChecksPassed || !firstName || !lastName || !email || !password} 
                />
                {!allChecksPassed && (
                  <p className="text-center text-[9px] font-black uppercase text-zinc-700 tracking-widest">All protocol rules must be affirmed to proceed</p>
                )}
              </div>
            </form>
          )}
...
const InputField = ({ label, type, value, onChange, icon: Icon, placeholder }: any) => (
  <div className="space-y-1">
    ...
    <div className="relative">
      ...
      <input 
        ...
        className="w-full bg-black/50 border border-zinc-800 rounded-lg py-3 pl-10 pr-4 outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/50 text-white mono text-xs transition-all"
      />
    </div>
  </div>
);

const SubmitButton = ({ label, icon: Icon, disabled }: any) => (
  <button 
    type="submit"
    disabled={disabled}
    className={`w-full py-4 bg-orange-500 text-black font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-orange-500/10 ${disabled ? 'opacity-20 cursor-not-allowed grayscale' : 'hover:bg-white hover:scale-[1.01] active:scale-[0.99]'}`}
  >
    <Icon size={18} /> {label}
  </button>
);

const SelectionCard = ({ title, desc, icon: Icon, onClick, highlight }: any) => (
  <button 
    onClick={onClick}
    className={`p-6 border rounded-2xl text-left space-y-4 transition-all group ${highlight ? 'border-orange-500/30 bg-orange-500/5 hover:border-orange-500' : 'border-zinc-800 bg-zinc-900/50 hover:border-zinc-700'}`}
  >
...
const RuleCheck = ({ label, active, onToggle }: any) => (
  <div 
    onClick={onToggle}
    className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${active ? 'bg-orange-500/10 border-orange-500/30' : 'bg-black/30 border-zinc-800 hover:border-zinc-700'}`}
  >
